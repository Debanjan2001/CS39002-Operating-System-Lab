...
int getRandomFromRange(int lower_limit, int higher_limit) {
	int range = higher_limit - lower_limit + 1;
	return (rand() % range + lower_limit);
}

int sleepForMiliseconds(int ms_time) {
	struct timespec ts;
	if (ms_time < 0) {
        return -1;
	}
...
void* producer_thread_handler(void* param) {
    ...
    /** */
    int time_to_run = getRandomFromRange(10, 20);
    /** */
    ...
    while(1) {
        ...
        /** */
        int sleeptime = getRandomFromRange(200, 500);
        /** */

        sleepForMiliseconds(sleeptime);
    }  
        ...
        for(int i = count_jobs-1; i >= 0; i--) {
            ...
            if(shmem->job_queue[i].status == 0) {
                ...
                if(flag == 1) {
                    ...
                    int proc_time = shmem->job_queue[i].duration;
                    pthread_mutex_unlock(&(shmem->job_queue[i].mutex));
                    
                    sleepForMiliseconds(proc_time);
...
int main() {
    ...
    /** */
    int init_num_jobs = getRandomFromRange(300, 500);
    /** */
